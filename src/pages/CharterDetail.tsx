...
/** Legacy `duration` line; BaseListing has no duration. */
const charterDurationLabel = (l: BaseListing) => l.tags[0] ?? l.category ?? "Charter";

const charterCapacityLabel = (l: BaseListing) =>
...
const CharterDetail = () => {
  ...
  const charter = useMemo(() => {
    ...
    const l = getListingById(charterId);
    return l?.type === "charter" ? l : null;
  }, [charterId]);
      ...
      <div className="min-h-screen bg-background text-foreground flex flex-col">
        <header className="py-8 px-6">
          <div className="mx-auto max-w-7xl flex items-center gap-3">
            ...
            >
              ...
              <span className="text-xs uppercase tracking-[0.2em]">{t("detail_back_to_charters")}</span>
            </Link>
          </div>
        </header>
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 pb-24">
          <div className="rounded-2xl border border-border/40 bg-card p-8">
            <p className="luxury-subheading text-primary mb-4">{t("detail_charter_not_found_title")}</p>
            <h1 className="luxury-heading text-3xl md:text-4xl text-foreground mb-4">
              {t("detail_charter_missing_heading")}
            </h1>
            <p className="luxury-body text-muted-foreground/60 text-sm max-w-2xl">
              {t("detail_charter_not_found_copy")}
            </p>
            <div className="mt-8">
              <Link to="/charters" className="luxury-btn-bold inline-block">
                {t("common_browse_charters")}
              </Link>
            </div>
          </div>
        </main>
      ...
      <section className="relative h-[70vh] md:h-[78vh] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex items-end pb-14 md:pb-16 px-4 sm:px-6 lg:px-12">
          <div className="mx-auto max-w-7xl w-full grid lg:grid-cols-[1.4fr_0.6fr] gap-8 items-end">
            ...
            >
              <p className="luxury-subheading text-primary/70 mb-3">{t("common_charter")}</p>
              <h1 className="luxury-heading text-4xl md:text-5xl lg:text-6xl text-foreground mb-4">
                {charter.title}
              </h1>
              <p className="luxury-body text-foreground/70 text-sm max-w-2xl">{listingTagline(charter)}</p>
            </motion.div>
            ...
            >
              <div className="rounded-2xl border border-border/40 bg-background/40 backdrop-blur-md p-6">
                ...
                <div className="flex flex-wrap gap-3">
                  <span className="text-sm text-foreground/70">{charterDurationLabel(charter)}</span>
                  <span className="text-foreground/20">|</span>
                  <span className="text-sm text-foreground/70">{charterCapacityLabel(charter)}</span>
                </div>
              </div>
            </motion.div>
          </div>
        ...
        <div className="grid lg:grid-cols-[1.5fr_0.5fr] gap-10 lg:gap-12 items-start">
          <section>
            ...
            >
              ...
              <p className="luxury-body text-muted-foreground/70 text-sm max-w-2xl">{charter.description}</p>
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {listingHighlights(charter).map((h) => (
                    <div
                      key={h}
          ...
          <aside className="lg:sticky lg:top-24">
            ...
            >
              ...
              >
                {t("common_explore_more_charters")}
              </Link>
            </div>
          </aside>
        </div>
